...
#define MAX_FILENAME 50 // Maximum length of filename
...
int scanFile(FILE *file)
{
    ...
    // For example, you can read each line and process its content

    char line[100];
    while (fgets(line, sizeof(line), file))
    {
        // Process each line of the file
        // Example: Extract data from the line and perform operations

        // Assuming the file contains name, midterm grade, and final grade separated by spaces
        char name[50];
...
int savingToFile(FILE *file, struct grade_t *grades, int num_records)
{
    ...
    fprintf(file, "\t%-25s\t%-10s\t%-12s\t%-9s\n", "Name", "Midterm", "Final Grade", "Ave.grade");
    for (int i = 0; i < num_records; i++)
    {
        // fprintf(file, "%s %.2f %.2f\n", grades[i].name, grades[i].midterm_grade, grades[i].final_grade);
        float ave_grade = (grades[i].midterm_grade + grades[i].final_grade) / 2;

        fprintf(file, "\t%-25s\t%-10.2f\t%-12.2f\t%-9.2f\n", grades[i].name, grades[i].midterm_grade, grades[i].final_grade, ave_grade);
    }
